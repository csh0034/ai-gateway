...
> ⚠️ **유지보수 정체**: PyPI 최신은 `0.2.0` (2024-07), GitHub 마지막 코드 push 는 2024-08 로 약 1년 9개월간 업데이트가 없습니다 (아카이브 처리는 안 됨). 마지막 커밋이 `pin dep numpy<2` 라 **numpy 2.x 환경에서 설치 충돌 가능** — 예제는 numpy 1.x 가상환경 권장. 논문(ICLR 2025) 레퍼런스로는 유효하나 운영 도입 대상으로는 부적합합니다.
